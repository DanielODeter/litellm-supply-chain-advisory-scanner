...
echo   deploy.bat org ^<org-id^> [email] [profile] [regions]  - AWS Organizations deployment
echo.
echo Examples:
echo   deploy.bat single your@email.com my-profile
echo   deploy.bat org o-abc123xyz your@email.com my-profile "us-east-1,us-west-2"
goto end
...
if "%ORG_ID%"=="" (
    ...
    echo Usage: deploy.bat org ^<org-id^> [email] [profile] [regions]
    goto end
)
...
if "%EMAIL%"=="" (
    aws cloudformation deploy ^
        ...
        --region %REGION% ^
        --profile %PROFILE% ^
        --parameter-overrides OrganizationId=%ORG_ID% DeploymentRegions=%REGIONS%
) else (
    aws cloudformation deploy ^
        --template-file scanner-org.yaml ^
        --stack-name %STACK% ^
        ...
        --region %REGION% ^
        --profile %PROFILE% ^
        --parameter-overrides OrganizationId=%ORG_ID% NotificationEmail=%EMAIL% DeploymentRegions=%REGIONS%
)
